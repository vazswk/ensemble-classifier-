function  [testing_errors, testing_error, std_testing_error]= my_tutorial(cover_path,stego_path,name_ensemble) 
% -------------------------------------------------------------------------
...
% quality factor = 75.
% cover_path ='C:\Users\VAZ\Desktop\jpeg ca\test_new_results\test_new_results\comp_200-num-1500\pev\pev_H\cover_H';
% stego_path ='C:\Users\VAZ\Desktop\jpeg ca\test_new_results\test_new_results\comp_200-num-1500\pev\pev_H\NON_ORI_stego_10';
cover = load(cover_path);
stego = load(stego_path);
%stego = load('C:\Users\VAZ\Desktop\jpeg ca\test_results_STC\feature extraction\pev\new\AVG_Distribution_stego_1-1000_rayligh_40');
...
names = sort(names);   %为了避免载体和隐写体顺序不同，对其进行重新排序

% Prepare cover features C
%%%%%%%%***********************%%%%%%%%
% num_1= [1 5 7 2 8 4 9 21 87 29];
% num_2 = [5 8 4 29 87];
% num_1(ismember(num_1,num_2));
% ans =
% 
% 5     8     4    87    29
%%%%%%%%***********************%%%%%%%%
...
[cover_names,ix] = sort(cover_names); %对特征样本进行排序，得到排序后的名称和序号
C = cover.F(ismember(cover.names,names),:);  %ismember返回的是坐标
C = C(ix,:);  %使得载体特征样本的名称names和数值F一一对应
...
S = S(ix,:); %使得隐写体特征样本的名称names和数值F一一对应
...
RandStream.setGlobalStream(RandStream('mt19937ar','Seed',1)); %创建伪随机序列

% Division into training/testing set (half/half & preserving pairs)
random_permutation = randperm(size(C,1));
training_set = random_permutation(1:round(size(C,1)/2));   %一半的载体特征样本用来训练
testing_set = random_permutation(round(size(C,1)/2)+1:end);%一半的载体特征样本用来测试
training_names = names(training_set);
testing_names = names(testing_set);
%%%%%%%%%%%%%%%********************************%%%%%%%%%%%%%%%
% %  上述程序的效果测试如下：
% %  names= [1 5 7 2 8 4 9 21 87 29];size(C)=10;
% % random_permutation =
% % 
% %      5     7     3    10     4     2     9     6     1     8
% % 
% % training_names =
% % 
% %      8     9     7    29     2
...
[trained_ensemble,results] = ensemble_training(TRN_cover,TRN_stego);  %训练得到的结果
...
figure(1);
clf;
plot(results.search.d_sub,results.search.OOB,'.b');hold on;
...
%% 表决，投票的分析
% 通过分析，votes的判决阈值为0；
% 至于为什么是这个值，猜想，在训练阶段，训练器就设置判决值为0，然后不断的调整训练参数，最终得到一个最优的结果
% 即找到一个虚警率，使得总体的检测误差最小
% 对于ensemble_testing测试输出的votes结果，如果在判决阈值为0时，得到了最终的虚警率和漏检率，接着可以通过调整
% 不同的阈值，得到不同的虚警率，及其漏检率，最终可以得到ROC曲线下的各个点的数值。

% Predictions: -1 stands for cover, +1 for stego
false_alarms = sum(test_results_cover.predictions~=-1);  %  载体被错误报警：false_alarms
missed_detections = sum(test_results_stego.predictions~=+1);%  隐写体没有报警：missed_detections
num_testing_samples = size(TST_cover,1)+size(TST_stego,1);
testing_error = (false_alarms + missed_detections)/num_testing_samples;  %得到分类错误率
...
settings = struct('d_sub',results.optimal_d_sub,'L',30,'seed_subspaces',5,'seed_bootstrap',73);
[~,results] = ensemble_training(TRN_cover,TRN_stego,settings);

% You can fully suppress the ensemble output by setting 'verbose' to 0:
settings = struct('d_sub',results.optimal_d_sub,'L',30,'verbose',0);
[~,results] = ensemble_training(TRN_cover,TRN_stego,settings);
...
settings = struct('d_sub',results.optimal_d_sub,'L',30,'verbose',2);
[~,results] = ensemble_training(TRN_cover,TRN_stego,settings);
...
% fix d_sub to results.optimal_d_sub:
...
%% 将分类错误的样本名进行存储
TST_cover_names = cover_names(testing_set);
TST_stego_names = stego_names(testing_set);

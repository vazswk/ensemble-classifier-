function  [testing_errors, testing_error, std_testing_error]= my_tutorial_ROC(cover_path,stego_path,name_ensemble) 
% -------------------------------------------------------------------------
...
names = intersect(cover.names,stego.names); % 为了避免载体和隐写体数目，求其交集
...
legend(sprintf('AUC = %.4f',auc));  % AUC是对ROC曲线的量化指标
...
num_testing_samples = size(TST_cover,1)+size(TST_stego,1);
testing_errors = (false_alarms + missed_detections)/num_testing_samples;
